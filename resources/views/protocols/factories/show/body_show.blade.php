<div class="content">
    ...
    foreach ($areas_firm as $show) {
        if ($show->district_id == $protocol->district_id) {
            $district_firm = $show->name;
        }
    }
    ///// ЗА ОБЕКТИТЕ
    foreach ($areas_all as $district) {
        if ($district->id == $protocol->area_object) {
            $area_object = $district->areas_name;
        }
    }
    foreach ($areas_object as $show){
    ...
    if($protocol->ot == 200){
        $type_object = 'ПРОИЗВОДИТЕЛ НА ПРЗ';
    }
    ...
    <p>Представлявано от: <span class="bold"> {!! $owner !!}</span> с ЕГН: <span class="bold"> {!! $protocol->pin_owner !!}</span></p>
    <br>
    ...
    <div class="col-md-12 my_div " id="prz_check">
        <hr class="middle_hr">
        {!! Form::open(['url'=>'assay-prz-factory/add/'.$protocol->id , 'method'=>'POST', 'id'=>'form-prz']) !!}
