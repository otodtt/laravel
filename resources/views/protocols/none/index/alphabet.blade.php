...
for ($i = 0 ;  $i <$ic ; ++$i) {
    ...
    foreach ($alphabet as $v){
        if($v === $i){
            if($sort_abc == $i){
                echo '<a href="/протоколи-обекти/сортирай/'.$i.'/'.$start_years.'/'.$end_years.'/'.$sort_areas_return.'/'.$sort_inspector_return.'/'.$sort_assay_return.'
...
<div class="wrap_alphabet_reset">
    <a href="/протоколи-обекти/сортирай/0/{!! $start_years !!}/{!! $end_years !!}/{!! $sort_areas_return !!}/{!! $sort_inspector_return !!}/{!! $sort_assay_return !!}
