...
if(Input::has('start_year') || Input::has('end_year') || Input::has('inspector_sort') || Input::has('abc')
        || Input::has('assay_sort') || Input::has('object_sort')){
    ...
    $sort_abc = $abc;
    if(isset($years_start_sort) || isset($years_end_sort) || isset($sort_inspector) || isset($sort_assay)
            || isset($sort_assay) || isset($sort_object)){
...
for ($i = 0 ;  $i <$ic ; ++$i) {
    ...
    foreach ($alphabet as $v){
        if($v === $i){
            if($sort_abc == $i){
                echo '<a href="/други-обекти/сортирай/'.$i.'/'.$start_years.'/'.$end_years.'/'.$sort_inspector_return.'/'.$sort_assay_return.'/'.$sort_object_return.'
...
?>
    ...
    <a href="/други-обекти/сортирай/0/{!! $start_years !!}/{!! $end_years !!}/{!! $sort_inspector_return !!}/{!! $sort_assay_return !!}/{!! $sort_object_return !!}
