<?php
if (isset($pharmacy->date_certificate)) {
    if ($pharmacy->date_petition == 0) {
        $date_certificate = '';
    } else {
        $date_certificate = $pharmacy->date_certificate;
    }
} else {
    $date_certificate = null;
}

$date_petition = date('d.m.Y', $pharmacy->date_change);
$date_edition = date('d.m.Y', $pharmacy->date_edition);
...
$number_petition = $pharmacy->number_change;

if (isset($pharmacy->certificate) && ($pharmacy->certificate !== 0)) {
        ...
        <div class="col-md-5">
            <fieldset class="small_field">
                ...
                <legend class="small_legend">Номер на {{ $raz_ud }}</legend>
                    <span class="bold">{!! $raz_ud !!} № {{$pharmacy->index_licence}} - {{$number_view}}
                        / {{date('d.m.Y',$pharmacy->date_licence)}} г.</span>
            </fieldset>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-md-5">
                <fieldset class="small_field">
                    ...
                    {!! Form::select('user_change', $inspectors, $pharmacy->id_user_change, ['id' =>'id_user',
