<?php
    ...
    for ($i = 0 ;  $i <$ic ; ++$i) {
        ...
        foreach ($alphabet as $v){
            if($v === $i){
                if($sort_abc == $i){
                    echo '<a href="/складове/сортирай/'.$i.'/'.$sort_areas.'/'.$sort_years.'/'.$sort_licence.'  " class="alfabet, ,alfmark" name="abc" >[ '.$alphabet_bg[$i].' ]</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
                else{
                    echo '<a href="/складове/сортирай/'.$i.'/'.$sort_areas.'/'.$sort_years.'/'.$sort_licence.' " class="alfabet" name="abc">'.$alphabet_bg[$i].'</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
            }
    ...
    <div class="wrap_alfrecet">
        <a href="/складове/сортирай/0/<?php echo $sort_areas; ?>/<?php echo $sort_years; ?>/<?php echo $sort_licence; ?>" class="fa fa-list-ol btn btn-primary my_btn" name="abc" > Всички</a>
    </div>
</div>