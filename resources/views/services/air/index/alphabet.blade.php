<?php
    ...
    for ($i = 0 ;  $i <$ic ; ++$i) {
        ...
        foreach ($alphabet as $v){
            if($v === $i){
                if($sort_abc == $i){
                    echo '<a href="/въздушни/сортирай/'.$i.'/'.$sort_inspector_return.'/'.$year_now.'  " class="alphabet alphabet_mark" name="abc" >[ '.$alphabet_bg[$i].' ]</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
                else{
                    echo '<a href="/въздушни/сортирай/'.$i.'/'.$sort_inspector_return.'/'.$year_now.' " class="alphabet" name="abc">'.$alphabet_bg[$i].'</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
            }
    ...
    <div class="wrap_alphabet_reset">
        <a href="/въздушни/сортирай/0/{!! $sort_inspector_return !!}/{!! $year_now !!}" class="fa fa-list-ol btn btn-primary my_btn" name="abc" > Всички</a>
    </div>
</div>