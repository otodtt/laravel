...
for ($i = 0 ;  $i <$ic ; ++$i) {
    ...
    foreach ($alphabet as $v){
        if($v === $i){
            if($sort_abc == $i){
                echo '<a href="/протоколи-нарушения/сортирай/'.$i.'/'.$start_years.'/'.$end_years.'/'.$sort_object_return.'/'.$sort_inspector_return.'
...
<div class="wrap_alphabet_reset">
    <a href="/протоколи-нарушения/сортирай/0/{!! $start_years !!}/{!! $end_years !!}/{!! $sort_object_return !!}/{!! $sort_inspector_return !!}
