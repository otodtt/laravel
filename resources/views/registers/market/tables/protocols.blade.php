<table class="tableall" id="testTable"  border="1">
    ...
    <tr>
        ...
        <th style="width: 280px" class="middle">Проверявано лице/фирма</th>
        <th style="width: 100px" class="middle">Вид и адрес на обекта<br/><span class="small">(ССА, склад, цех, нерегл.обект)</span></th>
    ...
    @foreach($all_protocols as $protocol)
        ...
        elseif($protocol['ot'] == 100){
            $type_object = 'Не Рег. обект';
        }
        elseif($protocol['ot'] == 200){
            $type_object = 'П-л ПРЗ';
        }
        else{
            $type_object = ' ';
        ...
        <tr >
            <td>{{ $n++ }}</td>
            <td>{{ $protocol['number'] }}/{{ date('d.m.Y', $protocol['date_protocol']) }}</td>
            <td class="">{{ $protocol['inspector_name'].' - '.$protocol['position_short'] }}</td>
            <td class="">{{ $full_name }}</td>
            <td class="middle" >{{ $type_object }}</td>
