<?php
    ...
    for ($i = 0 ;  $i <$ic ; ++$i) {
        ...
        foreach ($alphabet as $v){
            if($v === $i){
                if($sort_abc == $i){
                    echo '<a href="/сертификати/сортирай/'.$i.'/'.$start_years.'/'.$end_years.'/'.$sort_limit_return.'/'.$sort_inspector_return.'  " class="alphabet alphabet_mark" name="abc" >[ '.$alphabet_bg[$i].' ]</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
                else{
                    echo '<a href="/сертификати/сортирай/'.$i.'/'.$start_years.'/'.$end_years.'/'.$sort_limit_return.'/'.$sort_inspector_return.' " class="alphabet" name="abc">'.$alphabet_bg[$i].'</a>&nbsp;&nbsp;';
                    $keys = $i;
                    break;
                }
            }
    ...
    <div class="wrap_alphabet_reset">
        <a href="/сертификати/сортирай/0/{!! $start_years !!}/{!! $end_years !!}/{!! $sort_limit_return !!}/{!! $sort_inspector_return !!}" class="fa fa-list-ol btn btn-primary my_btn" name="abc" > Всички</a>
    </div>
</div>