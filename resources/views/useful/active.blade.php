            ...
            <table>
                ...
                @foreach($regulations as $regulation)
                    <tr style="border-bottom: 1px solid rgba(0, 0, 0, 0.45);">
                        ...
                        @if(Auth::user()->admin == 2 )
                            <td>
                                <form action="{{ url('/useful/document/destroy/'.$regulation->id) }}" method="post" style="display: inline-block" onsubmit="return confirm('Наистина ли искате да изтриете тoзи документ?');">
                                    <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-minus" aria-hidden="true"></i></button>
