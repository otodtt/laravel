...
class ChangePharmaciesRequest extends Request
{
    /**
    ...
    {
        $id = $this->segment(3);
        $type_ru = Pharmacy::findOrFail($id);
        $date_license = date('d.m.Y',$type_ru->date_licence);
        $data = null;
        if ($type_ru->raz_udost == 1){
            $data = [
                'active'=> 'required',
                'number_change'=> 'numeric|required|min:1',
    ...
    public function messages()
    {
        ...
        if($type_ru->raz_udost == 2) {
            $data = [
                ...
                'certificate.min' => 'Номер на Сертификата не може да нула 0!',
