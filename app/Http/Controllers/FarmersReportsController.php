...
class FarmersReportsController extends Controller
{
    private $logo = null;
    ...
    public function index()
    {
        ...
        $reports = FarmerReport::select('id', 'farmer_id', 'number_report', 'date_report', 'name', 'pin', 'district_object',
            'location', 'description', 'inspector', 'inspector_name', 'where_check', 'is_all')->orderBy('date_report', 'desc')->get();
        ...
        return view('farmers.reports.index', compact('reports', 'districts_list', 'districts_farm', 'alphabet','abc', 'type_opinion',
    ...
    public function create($id)
    {
        ...
        return view('farmers.reports.crud.create', compact('farmer', 'inspectors', 'regions', 'districts_farm', 'districts', 'checks'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function create_part($id, $idr, $part = null)
    {
        $farmer = Farmer::findOrFail($id);
        $report = FarmerReport::findOrFail($idr);
        ...
        return view('farmers.reports.crud.create', compact('farmer', 'inspectors', 'regions', 'districts_farm', 'districts', 'checks', 'report'));
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param Request|FarmerReportRequest|Requests\Request $request
     * @param  int $id
     * @return \Illuminate\Http\Response
     */
    public function store_first(FarmerReportRequest $request, $id)
    {
        $egn_eik = null;
        ...
        $date = substr($request['starting_time'], 0, -9);
        ...
        $data = [
            ...
            'where_check'=>$request['where_check'],
...
//dd($data);
        $report = new FarmerReport($data);
        $farmer->reports()->save($report);

        Session::flash('message', 'Протокола е добавен успешно!');
        return Redirect::to('/доклад-добави/'.$farmer->id.'/'.$report->id.'/1');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function update_first(FarmerReportRequest $request, $id)
    {
        $inspector_name_sql1 = User::where('id', '=', $request['inspector'])->get()->toArray();
        $inspector_name = $inspector_name_sql1[0]['short_name'];
        $position = $inspector_name_sql1[0]['position'];
        ...
        return Redirect::to('/доклад-добави/'.$report->farmer_id.'/'.$report->id.'/1');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function store_second(FarmerReportRequest $request, $id, $idr)
    {
        $farmer = Farmer::findOrFail($id);
        $report = FarmerReport::findOrFail($idr);
        ...
        $data = [
            ...
            'primaryR' => $request['primaryR'],
            'primary_note' => $request['primary_note'],
            'seeds' => $request['seeds'],
            'seeds_note' => $request['seeds_note'],
            ...
            'permit_note' => $request['permit_note'],
            'disclosure' => $request['disclosure'],
            'disclosure_note' => $request['disclosure_note'],
            'spraying' => $request['spraying'],
            'spraying_note' => $request['spraying_note'],
            'second' => 1,
            'is_all' => 2,
        ];
        ...
        Session::flash('message', 'Протокола е добавен успешно!');
        return Redirect::to('/доклад-добави/'.$farmer->id.'/'.$report->id.'/2');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function store_third(FarmerReportRequest $request, $id, $idr)
    {
        $farmer = Farmer::findOrFail($id);
        $report = FarmerReport::findOrFail($idr);
        ...
        $data = [
            ...
            'expiry_note' => $request['expiry_note'],
            'allocation' => $request['allocation'],
            'allocation_note' => $request['allocation_note'],
            'metal' => $request['metal'],
            'metal_note' => $request['metal_note'],
            'empty' => $request['empty'],
            'empty_note' => $request['empty_note'],
            'third' => 1,
            'is_all' => 3,
        ];
        ...
        Session::flash('message', 'Протокола е добавен успешно!');
        return Redirect::to('/доклад-добави/'.$farmer->id.'/'.$report->id.'/3');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function store_fourth(FarmerReportRequest $request, $id, $idr)
    {
        $farmer = Farmer::findOrFail($id);
        $report = FarmerReport::findOrFail($idr);
        ...
        $data = [
            ...
            'relevant_note' => $request['relevant_note'],
            'concentration' => $request['concentration'],
            'concentration_note' => $request['concentration_note'],
            'phenophase' => $request['phenophase'],
            'phenophase_note' => $request['phenophase_note'],
            'distances' => $request['distances'],
            'distances_note' => $request['distances_note'],
            'buildings' => $request['buildings'],
            'buildings_note' => $request['buildings_note'],
            'watersheds' => $request['watersheds'],
            'watersheds_note' => $request['watersheds_note'],
            'irrigation' => $request['irrigation'],
            'irrigation_note' => $request['irrigation_note'],
            'protected' => $request['protected'],
            'protected_note' => $request['protected_note'],
            'cleaning' => $request['cleaning'],
            'cleaning_note' => $request['cleaning_note'],
...
//        dd($data);
        ...
        Session::flash('message', 'Протокола е добавен успешно!');
        return Redirect::to('/доклад-добави/'.$farmer->id.'/'.$report->id.'/4');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function store_fifth(FarmerReportRequest $request, $id, $idr)
    {
        $farmer = Farmer::findOrFail($id);
        $report = FarmerReport::findOrFail($idr);
...
//        dd($data);
     ...
     * Store a newly created resource in storage.
     *
     * @param FarmerProtocolRequest|Requests\FarmerProtocolRequest $request
     * @param  int $id
     * @return \Illuminate\Http\Response
     */
    ...
    public function farmer_request(Request $request)
    {
        ...
        return view('farmers.reports.search.search', compact('firm', 'name', 'eik', 'gender', 'pin', 'name_firm', 'farmers'));
    }
            ...
            foreach($farmers as $farmer) {
                ...
                <ul>
                    <li>
                        ...
                        <div style='width: 50%; display: inline-block'><span><a href='/доклад-добави/$farmer->id' class='fa fa-address-card-o btn btn-warning my_btn'> ДОБАВИ ДОКЛАД ЗА ПРОВЕРКА НА ТОЗИ ЗС!</a></span></div>
                    </li>
                    <hr/>
                </ul>";
            }
        ...
        if(count($farmers)>0){
            foreach($farmers as $farmer) {
                ...
                <ul>
                    <li>
                        <div style='width: 40%; display: inline-block'><span class='bold' style='font-size: 1em;'>$farmer->name с ЕГН: $farmer->pin</span></div>
                        <div style='width: 50%; display: inline-block'><span><a href='/доклад-добави/$farmer->id' class='fa fa-address-card-o btn btn-warning my_btn'> ДОБАВИ ДОКЛАД ОТ ПРОВЕРКА НА ТОЗИ ЗС!</a></span></div>
                    </li>
                    <hr/>
                </ul>";
            }
