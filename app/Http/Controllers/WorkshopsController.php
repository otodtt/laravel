...
use odbh\Http\Requests\WorkshopUpdateRequest;
...
class WorkshopsController extends Controller
{
    ...
    {
        $abc = null;
        $alphabet = Workshop::where('end_date','>',time())->where('active','=',0)->lists('alphabet')->toArray();

        $districts = $this->objects_districts_list;

        $workshops = Workshop::where('end_date','>',time())->where('active','=',0)->orderBy('alphabet', 'asc')->get();
        return view('objects.workshops.index', compact('workshops', 'districts', 'alphabet','abc'));
    }

    /**
     * Сортиране на Цеховете
     *
     * @param  int $abc_list
    ...
    public function sort($abc_list = null, $areas_list = null, $years_list = null, $licence_list = null){
        ...
        if ((!array_key_exists($areas_sort, $districts) || $years_sort >= 6 || $licence_sort >= 3)
            ...
            $workshops = DB::select("SELECT * FROM `workshops` WHERE end_date > '.time().' AND active = 0 $areas_sql $years_sql $licence_sql $abc_sql " );
        }
        return view('objects.workshops.index', compact('workshops', 'districts', 'alphabet','abc', 'areas_sort', 'years_sort', 'licence_sort'));
    }
    ...
    public function create($id)
    {
        ...
        $last_number = Workshop::select('number_licence')
            ->where('raz_udost', '=', 2)
            ->orderBy('number_licence', 'desc')
            ->limit(1)->get()->toArray();

        return view('objects.workshops.create_workshop', compact('firm', 'districts', 'districts_firm_show', 'areas_firm', 'locations',
    ...
    public function store(PharmaciesRequest $request)
    {
        ...
        $data = ([
            'index_licence' => (int)$index_ur[0]['area_id'] . '2',
        ...
        $firm_many->workshops()->save($workshop);

        Session::flash('message', 'Цеха и Удостоверението са добавени успешно!');
    ...
    public function edit($firm_id, $id, $admin = null)
    {
        ...
        $inspectors_db = Workshop::where('inspector','!=',0)->lists('list_name', 'inspector')->toArray();
     ...
     *
     * @param  \odbh\Http\Requests\WorkshopUpdateRequest $request
     * @param  int $id
     * @return \Illuminate\Http\Response
     */
    public function update(WorkshopUpdateRequest $request, $id)
    {
        ...
        if($request['admin'] == 1){
            Session::flash('message', 'Цеха e редактиран успешно!');
            return Redirect::to('/фирма/'.$workshop->firm_id);
        }
        if($request['admin'] == 0){
            Session::flash('message', 'Удостоверението на Цеха e редактирано успешно!');
            return Redirect::to('/цех-удостоверение/'.$workshop->firm_id.'/'.$workshop->id);
        }
    }
    ...
    public function create_permits($id)
    {
        ...
        if($lock_permit[0]['lock_permit'] == 0) {
            ...
            return view('objects.workshops.create_permit', compact('firm', 'districts', 'districts_firm_show', 'areas_firm', 'locations',
                'index', 'firm_id'));
        }
        else{
    ...
    public function store_permit(PermitRequest $request, $id)
    {
        ...
        if($lock_permit[0]['lock_permit'] == 0) {
            ...
            Session::flash('message', 'Цеха и Разрешителното са добавени успешно!');
            return Redirect::to('/фирма/' . $id);
        }
        else{
            return Redirect::to('/фирма/' . $id);
    ...
    public function add($firm_id, $id)
    {
        ...
        return view('objects.workshops.add_workshop', compact('workshop', 'firm', 'districts', 'districts_firm_show', 'areas_firm',
